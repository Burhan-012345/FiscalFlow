...
def setup_admin(app):
    ...
    class CustomerModelView(SecureModelView):
        ...
        form_columns = ['name', 'email', 'phone', 'category', 'credit_limit', 'account_id']
    ...
    def add_transaction(customer_id):
        ...
        if form.validate_on_submit():
            ...
            notification = Notification(
                ...
                message=f'{transaction.type.replace("_", " ").title()} of ${transaction.amount:.2f} for {customer.name}',
                type='info'
            )
            db.session.add(notification)
            db.session.commit()
            ...
            # Check credit limit
            if customer.credit_limit > 0 and customer.current_balance > customer.credit_limit:
                warning_notification = Notification(
                    user_id=current_user.id,
                    title='Credit Limit Exceeded',
                    message=f'Customer {customer.name} has exceeded their credit limit of ${customer.credit_limit:.2f}',
                    type='warning'
                )
                db.session.add(warning_notification)
                db.session.commit()
                flash(f'Warning: {customer.name} has exceeded their credit limit!', 'warning')
            
            flash('Transaction added successfully!', 'success')
            return redirect(url_for('customer_detail', customer_id=customer_id))
        
        return render_template('dashboard/transactions.html', form=form, customer=customer)
    ...
    def download_template(template_type):
        ...
        try:
            if template_type == 'customers':
                ...
                headers = ['name', 'email', 'phone', 'category', 'credit_limit', 'current_balance']
                ...
                for customer in customers:
                    data_rows.append([
                        ...
                        str(customer.credit_limit or '0.00'),
                ...
                headers = [
                    ...
                    'customer_credit_limit', 'customer_current_balance',
...
def validate_csv_data(rows, import_type, user_id):
    ...
    for i, row in enumerate(data_rows, start=2):  # Start at 2 for line numbers (header is line 1)
        ...
        if import_type == 'customers':
            ...
            credit_limit = row_dict.get('credit_limit', '').strip()
            if credit_limit:
                try:
                    float(credit_limit)
                except ValueError:
                    row_errors.append(f"Row {i}: Credit limit must be a valid number")
